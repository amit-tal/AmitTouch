(function(){
  ...
  function ensureField(){
    ...
    if(!wrap){
      ...
      wrap.innerHTML='<span class="field-icon admin-code-icon"></span><input id="adminCode" class="field" inputmode="numeric" autocomplete="one-time-code" maxlength="4" placeholder="קוד מנהל">';
  ...
  function syncField(){
    ...
    if(!show){const code=document.getElementById('adminCode');if(code)code.value='';}
  }
  function install(){
    ...
    ensureField();
    name.addEventListener('input',syncField);
    phone.addEventListener('input',syncField);
    name.addEventListener('change',syncField);
    phone.addEventListener('change',syncField);
    syncField();
    ...
    window.login=async function(){
      ...
      if(fullName!==ADMIN_NAME||clean!==ADMIN_PHONE){
        const wrap=document.getElementById('adminCodeWrap');if(wrap)wrap.style.display='none';
