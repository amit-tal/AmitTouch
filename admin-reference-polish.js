(function(){
'use strict';
const old=document.getElementById('admin-reference-polish-v2');if(old)old.remove();
const style=document.createElement('style');
style.id='admin-reference-polish-v3';
style.textContent=`
:root{--ref-green:#173f3b;--ref-green-soft:#315f5a;--ref-cream:#fbf6f0;--ref-pink:#efd7d1;--ref-pink-soft:#f7ebe7;--ref-muted:#858d89;--ref-border:rgba(255,255,255,.94);--ref-shadow:0 8px 24px rgba(68,69,64,.055)}
...
body.admin-session-active .af.ar-home-ref .af-head .af-icon:last-child:before{content:'♧';font-size:15px;opacity:.9}
...
body.admin-session-active .af.ar-home-ref .af-stat span{font-size:7.6px!important;line-height:1.15!important;color:#6f7975!important}
...
body.admin-session-active .af.ar-home-ref .ar-next-photo{width:52px!important;height:52px!important;border-radius:50%!important;background:#efe1da url('/assets/WhatsApp%20Image%202026-08-15%20at%2020.58.05.jpeg') center/cover no-repeat!important;border:2px solid rgba(255,255,255,.92)!important;box-shadow:0 4px 13px rgba(61,65,61,.08)!important}
...
body.admin-session-active .af.ar-home-ref .ar-next-time{font-size:15px!important;font-weight:700!important;color:var(--ref-green)!important;text-align:left!important;direction:ltr!important}
...
@media(max-width:355px){body.admin-session-active #amitAdminRoot #adminBody{padding-left:12px!important;padding-right:12px!important}}
`;
document.head.appendChild(style);

const icons={customers:'<svg viewBox="0 0 24 24"><circle cx="12" cy="8" r="3.4"/><path d="M5.8 20c.7-4 2.8-6 6.2-6s5.5 2 6.2 6"/></svg>',messages:'<svg viewBox="0 0 24 24"><path d="M5 5.5h14v10H9l-4 3v-13Z"/><path d="M9 9h6M9 12h4"/></svg>',home:'<svg viewBox="0 0 24 24"><path d="m4 11 8-7 8 7v9h-5v-6H9v6H4v-9Z"/></svg>',calendar:'<svg viewBox="0 0 24 24"><rect x="4" y="6" width="16" height="14" rx="2"/><path d="M8 3v5M16 3v5M4 10h16M8 14h.01M12 14h.01M16 14h.01"/></svg>',manage:'<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M19 13.5v-3l-2-.6-.6-1.4 1-1.8-2.1-2.1-1.8 1L12 5l-.6-2h-3l-.6 2-1.5.6-1.8-1-2.1 2.1 1 1.8-.6 1.4-2 .6v3l2 .6.6 1.4-1 1.8 2.1 2.1 1.8-1 1.5.6.6 2h3l.6-2 1.5-.6 1.8 1 2.1-2.1-1-1.8.6-1.4 2-.6Z"/></svg>'};
let lastRoot=null,lastSignature='';
function apply(){
 const root=document.getElementById('adminBody');if(!root)return;
 const af=root.querySelector('.af');if(!af)return;
 const title=af.querySelector('.af-title');const isHome=!!(title&&title.textContent.includes('ערב טוב עמית'));
 af.classList.toggle('ar-home-ref',isHome);
 const nav=root.querySelector('.af-nav');if(nav){nav.querySelectorAll('button[data-route]').forEach(btn=>{const icon=btn.querySelector('i'),svg=icons[btn.dataset.route];if(icon&&svg&&icon.dataset.refIcon!==btn.dataset.route){icon.innerHTML=svg;icon.dataset.refIcon=btn.dataset.route}})}
 if(!isHome)return;
 const signature=af.innerText.slice(0,500);if(root===lastRoot&&signature===lastSignature&&af.dataset.refDone==='1')return;lastRoot=root;lastSignature=signature;
 const grid=af.querySelector('.af-grid');if(grid&&!grid.querySelector('.ar-stat-month')){const stats=[...grid.querySelectorAll('.af-stat')],income=stats.find(x=>x.textContent.includes('הכנסות השבוע'));if(income)income.classList.add('ar-stat-income');const month=document.createElement('div');month.className='af-card af-stat ar-stat-month';month.innerHTML='<b>'+stateMonthCount()+'</b><span>תורים החודש</span>';if(income)grid.insertBefore(month,income);else grid.appendChild(month)}
 const sections=[...af.querySelectorAll('.af-section')];const nextSection=sections.find(x=>x.textContent.trim()==='התור הבא');if(nextSection){const card=nextSection.nextElementSibling;if(card&&card.classList.contains('af-card')&&!card.classList.contains('ar-next-card')&&!card.classList.contains('af-note')){card.classList.add('ar-next-card');const h3=card.querySelector('h3'),p=card.querySelector('.af-note'),actions=card.querySelector('.af-actions');if(h3&&p){const parts=h3.textContent.split('·').map(s=>s.trim()),time=parts.shift()||'',name=parts.join(' · ')||'לקוחה',pp=p.textContent.split('·').map(s=>s.trim()),service=pp[0]||'טיפול',minutes=pp.slice(1).join(' · ')||'';const top=document.createElement('div');top.className='ar-next-top';top.innerHTML='<div class="ar-next-photo"></div><div class="ar-next-copy"><strong>'+name+'</strong><span>'+service+'</span><small>'+minutes+'</small></div><div class="ar-next-time">'+time+'</div>';h3.remove();p.remove();card.insertBefore(top,actions||card.firstChild)}}}
 const todaySection=sections.find(x=>x.textContent.trim()==='היום שלי');if(todaySection){const list=todaySection.nextElementSibling;if(list&&list.classList.contains('af-list')&&!list.classList.contains('ar-today-card')){const rows=[...list.querySelectorAll('.af-row')],card=document.createElement('div');card.className='ar-today-card';if(!rows.length)card.innerHTML='<div class="ar-today-empty">אין תורים היום</div>';else rows.forEach(row=>{const h=row.querySelector('h3')?.textContent||'',p=row.querySelector('p')?.innerText||'',time=(h.match(/\d{1,2}:\d{2}/)||[''])[0],lines=p.split(/\n+/).map(s=>s.trim()).filter(Boolean),main=[lines[0]||'',(lines[1]||'').split('·')[0].trim()].filter(Boolean).join(' · '),line=document.createElement('div');line.className='ar-today-line';line.innerHTML='<div class="ar-today-main">'+main+'</div><div class="ar-today-time">'+time+'</div>';line.onclick=()=>row.click();card.appendChild(line)});list.replaceWith(card)}}
 af.dataset.refDone='1';
}
function stateMonthCount(){try{const appointments=window.__AMIT_ADMIN_APPOINTMENTS__||[];const now=new Date();return appointments.filter(a=>{const d=new Date(a.starts_at);return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear()&&a.status!=='cancelled'}).length}catch(_){return 0}}
let timer=0;function schedule(){clearTimeout(timer);timer=setTimeout(apply,30)}
const observer=new MutationObserver(muts=>{if(muts.some(m=>[...m.addedNodes].some(n=>n.nodeType===1&&(n.id==='adminBody'||n.querySelector?.('#adminBody')||n.classList?.contains('af')))))schedule()});
observer.observe(document.body,{childList:true,subtree:true});
window.addEventListener('amit:session-ready',schedule);window.addEventListener('pageshow',schedule);document.addEventListener('click',e=>{if(e.target.closest('[data-route],[data-back],[data-appt],[data-finish]'))schedule()},true);schedule();
})();