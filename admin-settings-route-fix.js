(function(){'use strict';
if(window.__AMIT_SETTINGS_ROUTE_FIX_V12__)return;
window.__AMIT_SETTINGS_ROUTE_FIX_V12__=true;
function loadScript(src,key){return new Promise((resolve,reject)=>{if(window[key])return resolve();const s=document.createElement('script');s.src=src+'?v=20260827-settings-v12';s.async=false;s.onload=()=>{window[key]=true;resolve()};s.onerror=reject;document.head.appendChild(s)})}
async function ensureCanonical(){
  await loadScript('/admin-settings-main-canonical.js','__AMIT_SETTINGS_MAIN_V12__');
  await loadScript('/admin-simple-settings.js','__AMIT_SIMPLE_SETTINGS_V12__');
  await loadScript('/admin-studio-settings-canonical.js','__AMIT_STUDIO_SETTINGS_V12__');
  await loadScript('/admin-studio-settings-theme-fix.js','__AMIT_STUDIO_THEME_FIX_V12__');
  await loadScript('/admin-settings-final-ui-fix.js','__AMIT_SETTINGS_FINAL_UI_V12__');
  await loadScript('/admin-settings-behavior-hooks.js','__AMIT_SETTINGS_HOOKS_V12__');
