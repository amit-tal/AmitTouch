(function(){
  ...
  if(!home)return;
  const BUILD='20260819-home-hero-fix-v14';
  ...
  function refresh(){
    ...
    if(photo){let art=photo.querySelector('.home-hero-art');if(!art){art=document.createElement('img');art.className='home-hero-art';art.alt='';photo.prepend(art);}art.src='/assets/home-hero.webp?v='+BUILD;}
    const copy=home.querySelector('.hero-copy');if(copy){const n=firstName();copy.innerHTML=`<span class="hero-greeting"><span class="hero-name">${n||'שם פרטי'}</span><span class="hero-welcome">ברוכה הבאה</span></span><strong class="hero-touch">לטאץ׳ שלך</strong><span class="home-heart" aria-hidden="true"></span>`;}
    const primary=home.querySelector('.primary');if(primary)primary.innerHTML=`<svg class="home-book-icon" viewBox="0 0 32 32" aria-hidden="true" fill="none"><rect x="4.5" y="6.5" width="23" height="21" rx="4" stroke="currentColor" stroke-width="2"/><path d="M10 4.5v5M22 4.5v5M5 12.5h22" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><circle cx="11" cy="17" r="1.5" fill="currentColor"/><circle cx="16" cy="17" r="1.5" fill="currentColor"/><circle cx="21" cy="17" r="1.5" fill="currentColor"/></svg><span>הזמיני תור</span>`;
  }
  refresh();new MutationObserver(()=>{if(home.classList.contains('active'))refresh()}).observe(home,{attributes:true,attributeFilter:['class']});
})();