...
app.get('/api/admin/appointments', async (_req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'ADMIN_APPOINTMENTS_FAILED' });
  }
});
  ...
  try {
    ...
    if (!isAdminCreated) {
      await supabase.from('admin_notifications').insert({
        ...
        title: 'תור ממתין לאישור',
        body: `${customer.first_name} ${customer.last_name} ביקשה ${booking.service}`,
        customer_id: customer.id,
        appointment_id: appointment.id,
        metadata: { starts_at: appointment.starts_at, status: 'pending' }
      });
    }
...
app.post('/api/admin/appointments/:appointmentId/reject', async (req, res) => {
  try {
    ...
    if (appointment.google_event_id) {
      try { await calendarClient().events.delete({ calendarId: CALENDAR_ID, eventId: appointment.google_event_id }); } catch {}
    }
    const reason = String(req.body?.reason || 'בקשת התור לא אושרה על ידי עמית');
    const { data: updated, error: updateError } = await supabase.from('appointments').update({ status: 'cancelled', cancelled_at: new Date().toISOString(), cancellation_reason: reason }).eq('id', appointment.id).select().single();
    if (updateError) throw updateError;
    res.json({ ok: true, appointment: updated });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'REJECTION_FAILED' });
  }
});
...
app.listen(port, '0.0.0.0', () => console.log(`AMIT TOUCH approval flow listening on ${port}`));
