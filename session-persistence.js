(function(){
  const KEY='amit-touch-signed-in-customer-v4';
  const OLD_KEYS=['amit-touch-signed-in-customer-v3','amit-touch-signed-in-customer-v2','amit-touch-signed-in-customer-v1'];
  const COOKIE='amit_touch_customer';
  ...
  function writeCookie(value){try{if(value)document.cookie=COOKIE+'='+encodeURIComponent(JSON.stringify(value))+'; Max-Age=31536000; Path=/; SameSite=Lax';else document.cookie=COOKIE+'=; Max-Age=0; Path=/; SameSite=Lax'}catch(_){}}
  function readCookie(){try{const part=document.cookie.split('; ').find(x=>x.startsWith(COOKIE+'='));return part?JSON.parse(decodeURIComponent(part.slice(COOKIE.length+1))):null}catch(_){return null}}

  function localRead(){
    if(window.__AMIT_EARLY_SESSION__?.id)return window.__AMIT_EARLY_SESSION__;
    try{
      const primary=localStorage.getItem(KEY);
      if(primary){const parsed=JSON.parse(primary);if(parsed?.id)return parsed}
      for(const old of OLD_KEYS){const raw=localStorage.getItem(old);if(raw){const parsed=JSON.parse(raw);if(parsed?.id)return parsed}}
    }catch(_){}
  ...
  function forceHome(saved){if(!saved?.id)return false;assignUser(saved);document.querySelectorAll('.screen').forEach(x=>x.classList.remove('active'));document.getElementById('home')?.classList.add('active');document.getElementById('nav')?.classList.add('show');try{window.enterApp?.()}catch(_){}window.renderHomeAppointments?.();window.renderNext?.();return true}
  ...
  window.logout=function(){explicitLogout=true;clearPersisted();assignUser(null);assignAppointments([]);document.getElementById('nav')?.classList.remove('show');if(typeof originalLogout==='function'){try{return originalLogout.apply(this,arguments)}catch(_){}}window.show?.('login')};
