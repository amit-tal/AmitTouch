...
function amitMessage(template, customer, appointment) {
  ...
  return String(template || '')
    .replaceAll('{שם}', name)
    .replaceAll('{תאריך}', start?.isValid ? start.toFormat('dd.MM.yy') : '')
    .replaceAll('{שעה}', start?.isValid ? start.toFormat('HH:mm') : '');
}
app.get('/api/admin/settings', async (_req, res) => {
  try {
    const supabase = supabaseClient();
    const settings = await amitGetSettings(supabase);
...
app.post('/api/admin/settings/pin', async (req, res) => {
  try {
    ...
    const { error } = await supabase.from('app_settings').update({ admin_pin: newPin, updated_at: new Date().toISOString() }).eq('id', 1);
    if (error) throw error;
    res.json({ ok: true });
...
app.get('/api/admin/customer-preview', async (_req, res) => {
  try {
    ...
    let { data: customer, error } = await supabase.from('customers').select('*').eq('phone', phone).maybeSingle();
    if (error) throw error;
    if (!customer) {
    ...
    res.json({ ok: true, customer: customerPayload ? customerPayload(customer) : { id: customer.id, firstName: customer.first_name, lastName: customer.last_name, fullName: ADMIN_NAME, phone: customer.phone, birthDate: customer.birth_date } });
...
source = source.replace(
  ...
  "if (fullName === ADMIN_NAME && phone === ADMIN_PHONE) { if (!adminCode) return res.status(403).json({ error: 'ADMIN_CODE_REQUIRED', requiresAdminCode: true }); const adminSettings = await amitGetSettings(supabaseClient()); if (adminCode !== String(adminSettings.admin_pin || ADMIN_CODE)) return res.status(401).json({ error: 'INVALID_ADMIN_CODE', requiresAdminCode: true }); return res.json({ ok: true, role: 'admin', customer: null }); }"
);

source = source.replace(
  "    const duration = Number(minutes) + BUFFER_MINUTES;",
  "    const supabase = supabaseClient(); const amitSettings = await amitGetSettings(supabase); const duration = Number(minutes) + Number(amitSettings.gap_minutes || 0);"
);
source = source.replace(
  "    const dayStart = DateTime.fromISO(`${date}T00:00`, { zone: TZ });",
  "    const dayStart = DateTime.fromISO(`${date}T00:00`, { zone: TZ }); const nowJerusalem = DateTime.now().setZone(TZ); const maxBookingDay = nowJerusalem.startOf('day').plus({ days: Number(amitSettings.booking_window_days || 90) }); if (dayStart > maxBookingDay) return res.json({ date, slots: [], bookingWindowExceeded: true });"
);
source = source.replace(
  "      const conflict = busy.some(item => conflictsWithBusy(t, end, item));",
  "      const tooSoon = t < nowJerusalem.plus({ hours: Number(amitSettings.min_advance_hours || 0) }); const conflict = tooSoon || busy.some(item => conflictsWithBusy(t, end, item));"
);
