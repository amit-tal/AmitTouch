(function(){
 ...
 const BUILD='20260819-home-appointments-v1';
 ...
 #home .appointment-heading:before,#home .appointment-heading:after{content:''!important;height:1px!important;flex:1!important;background:linear-gradient(90deg,transparent,rgba(7,88,79,.16),transparent)!important}
 #home .appointment-heading .diamond{font-size:15px!important;line-height:1!important;color:#07584f!important;opacity:.95!important}
 ...
 #home .appointment-status{width:48px!important;height:48px!important;border-radius:50%!important;background:rgba(248,218,211,.76)!important;display:grid!important;place-items:center!important}
 ...
 #home .appointment-meta{font-size:16px!important;line-height:1.35!important;font-weight:300!important;margin:8px 0 0!important;color:#151c1b!important}
 ...
 @media(max-width:360px){#home .appointment-card{grid-template-columns:46px minmax(0,1fr) 74px!important;padding:14px 12px!important;gap:10px!important}#home .appointment-service{font-size:18px!important}#home .appointment-meta{font-size:14px!important}}
 ...
 function card({day,date,time,service}){const el=document.createElement('div');el.className='appointment-card';el.innerHTML=`<div class="appointment-status" aria-hidden="true"><svg viewBox="0 0 32 32" fill="none"><path d="M8 16.5l5 5L24 10" stroke="#111" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg></div><div class="appointment-main"><div class="appointment-service">${service}</div></div><div class="appointment-date"><div class="appointment-day">${day}</div><div class="appointment-date-num">${date}</div><div class="appointment-time">${time}</div></div>`;return el;}
 function block(title,data){const b=document.createElement('section');b.className='appointment-block';const h=document.createElement('div');h.className='appointment-heading';h.innerHTML=`<span class="diamond">✦</span><span>${title}</span><span class="diamond">✦</span>`;b.append(h,card(data));return b;}
 host.append(block('תורים עתידיים',{day:'שלישי',date:'21.05',time:'15:30',service:'מניקור ג׳ל'}));
 host.append(block('הזמנה אחרונה',{day:'שלישי',date:'21.05',time:'15:30',service:'מניקור + ג׳ל לק'}));
